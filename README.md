...
`TAILSCALE_AUTH_KEY` | For **Tailscale** tunnel uses | Go to https://login.tailscale.com/admin/settings/keys and generate an auth key
...
4. Check the workflow logs for the Tailscale IP address assigned to the machine.

5. Open VNC Viewer (Download and install it), input the Tailscale IP to connect.
...
2. Second, check the workflow logs for the Tailscale IP address assigned to the machine.
3. Go to Windows Remote Desktop Connection app or Microsoft Remote Desktop software and connect using the Tailscale IP.
4. ENJOY!
...
3. Since we are using **Tailscale**, make sure all machines are on the same Tailscale network (tailnet) to access them.
