...
`TAILSCALE_AUTH_KEY` | 用於**Tailscale**隧道 | 前往 https://login.tailscale.com/admin/settings/keys 並生成一個認證密鑰
...
4. 檢查工作流程日誌以獲取分配給機器的Tailscale IP地址.
...
2.其次，檢查工作流程日誌以獲取分配給機器的Tailscale IP地址。\
3.轉到Windows遠程桌面連接應用程序或Microsoft遠程桌面軟件，使用Tailscale IP連接到Windows 10 VPS。\
4.享受！
...
3.由於我們使用的是 `Tailscale`，請確保所有機器都在同一個Tailscale網絡上以訪問它們。\
